...
function escapeMarkdownPreservingBold(text) {
  const placeholders = [];
  const withPlaceholders = String(text || '').replace(/\*\*([^*]+)\*\*/g, (_, inner) => {
    const token = `__FB_BOLD_${placeholders.length}__`;
    placeholders.push(`**${escapeBoldContent(inner)}**`);
    return token;
  });
  let escaped = escapeMarkdownText(withPlaceholders);
  placeholders.forEach((value, index) => {
    escaped = escaped.replace(`__FB_BOLD_${index}__`, value);
  });
  return escaped;
}

function buildTitleMarkdown(item) {
  const title = String(item.headline || item.title || '原文');
  const escapedTitle = escapeMarkdownPreservingBold(title);
  return title.includes('**') ? escapedTitle : `**${escapedTitle}**`;
}
...
function buildBodyMarkdown(item) {
  const body = normalizeParagraphMarkdown(
    item.body
    || item.detail
    || item.summary_text
  ...
  if (!body) return '';
  return `\n${escapeMarkdownPreservingBold(body)}`;
}

function detectSourceKind(item) {
  const sourceLabel = String(item?.source_label || '').toLowerCase();
  const identity = String(item?.person_identity || item?.identity || item?.role || '').toLowerCase();
  const profileUrl = String(item?.profile_url || item?.author_url || item?.source_url || item?.url || '').toLowerCase();

  if (/^x(\s|$|\/)/.test(sourceLabel) || sourceLabel.includes('twitter') || /x\.com\//.test(profileUrl)) {
    return 'x';
  }
  if (
    sourceLabel.includes('podcast')
    || identity.includes('podcast')
    || /youtube\.com|youtu\.be/.test(profileUrl)
  ) {
    return 'podcast';
  }
  if (
    sourceLabel.includes('blog')
    || identity.includes('blog')
    || /\/blog(\/|$)/.test(profileUrl)
  ) {
    return 'blog';
  }
  return 'other';
...
function wrapPurpleSummary(bodyMarkdown) {
  const normalizedBody = String(bodyMarkdown || '').replace(/^\n+/, '');
  return `📝 **Summary / 摘要**\n<font color='purple'>${normalizedBody}</font>`;
}
...
function buildMetaMarkdown(item) {
  const parts = [];
  const sourceKind = detectSourceKind(item);
  const sourceLabel = sourceKind === 'x'
    ? 'X / Twitter'
    : String(item.source_label || '').trim();
  const rawDate = String(item.posted_at || item.published_at || '').trim();
  const normalizedDate = /^\d{4}-\d{2}-\d{2}T/.test(rawDate) ? rawDate.slice(0, 10) : rawDate;

  if (sourceLabel) parts.push(escapeMarkdownText(sourceLabel));
  if (normalizedDate) parts.push(escapeMarkdownText(normalizedDate));
  return parts.join(' · ');
}
...
function buildCard(payload, avatarKeys = new Map()) {
  ...
  items.forEach((item, index) => {
    ...
    sections.forEach((section, sectionIndex) => {
      ...
      const sourceKind = detectSourceKind(item);
      let bodyMarkdown = buildBodyMarkdown(section);
      if (bodyMarkdown && (sourceKind === 'blog' || sourceKind === 'podcast')) {
        bodyMarkdown = wrapPurpleSummary(bodyMarkdown);
      }
      if (bodyMarkdown) {
        elements.push({
          tag: 'div',
      ...
      const hasMoreSectionsInItem = sectionIndex < sections.length - 1;
      const hasMoreItems = index < items.length - 1;
      if (hasMoreSectionsInItem || hasMoreItems) {
        elements.push({ tag: 'hr' });
      }
    });
  });
