...
function buildIncludedUserMap(includes) {
  const userMap = {};
...
function buildIncludedTweetMap(includes) {
  const tweetMap = {};
  ...
  for (const account of xAccounts) {
    ...
    try {
      const res = await fetch(
        ...
        `&tweet.fields=created_at,public_metrics,referenced_tweets,note_tweet,author_id` +
      ...
      const includedTweets = buildIncludedTweetMap(data.includes);
      const includedUsers = buildIncludedUserMap(data.includes);
      ...
      for (const t of allTweets) {
        ...
        const quotedTweetId = t.referenced_tweets?.find(r => r.type === 'quoted')?.id || null;
        const quotedTweetRecord = quotedTweetId ? includedTweets[quotedTweetId] : null;
        const quotedAuthor = quotedTweetRecord?.author_id ? includedUsers[quotedTweetRecord.author_id] : null;
        ...
        newTweets.push({
          ...
          quotedTweetId,
          quotedTweet: quotedTweetRecord ? {
            id: quotedTweetRecord.id,
            text: quotedTweetRecord.note_tweet?.text || quotedTweetRecord.text,
            url: quotedAuthor?.username
              ? `https://x.com/${quotedAuthor.username}/status/${quotedTweetRecord.id}`
              : `https://x.com/i/status/${quotedTweetRecord.id}`,
            authorName: quotedAuthor?.name || null,
            authorHandle: quotedAuthor?.username || null,
            authorUrl: quotedAuthor?.username ? `https://x.com/${quotedAuthor.username}` : null
          } : null
        });
