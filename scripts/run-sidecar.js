...
async function gatherRuntimeContext(config, state, args) {
  ...
  try {
    ...
    upstreamReady = Boolean(feeds?.loadMeta?.allRemote);
  } catch (error) {
  ...
  if (feeds?.loadMeta?.allRemote === false && feeds?.loadMeta?.remoteFailures?.length) {
    compatibilityWarnings.push(
      `Remote upstream feed fetch incomplete; local fallback was used for: ${feeds.loadMeta.remoteFailures.join(' | ')}`
    );
  }
...
async function execute(args) {
  ...
  if (!ctx.upstreamReady || ctx.freshnessMode !== 'commit') {
    await commitStateUpdate(async (state) => {
      state.lastCheckedAt = ctx.currentIso;
      state.lastFeedCompatibility = ctx.feedCompatibilitySummary;
      state.lastCompatibilityWarnings = [...ctx.compatibilityWarnings];
      ...
      state.lastEvaluatedOutcome = 'upstream_unavailable';
      await saveSidecarState(state);
    });
    return {
      status: 'skipped',
  ...
  if (!args.skipDelivery) {
    ...
    } else if (config.delivery.driver === 'lark_cli_feishu_card') {
      deliveryResult = await sendLarkCliFeishuCard(args.payloadPath, config);
    } else if (config.delivery.driver === 'feishu_webhook_card') {
      deliveryResult = await sendFeishuWebhookCard(args.payloadPath, config);
    } else {
